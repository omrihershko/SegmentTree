public class MaximumSegmentTreeByArray extends SegmentTreeByArray {
	...
	/**
	 * Constructs of MaximumSegmentTreeByArray
	...
	// left right is the range query
	//end start the rage of the arr

	/**
	 * A helper function that cuts the array and returns the largest value in the array
	 *
	 * @return int of the bigest num in the cut array
	 */
	...
	private int queryhelp(int node, int start, int end, int left, int right) {
		...
		tree[node] = Math.max(leftRec , rightRec);
		return (tree[node]);
	...
	public int build_tree(int[] arr, int node, int start, int end) {
		if (start == end) {
			...
			tree[node] = Math.max(build_tree(arr, (node * 2) + 1, start, midlle), build_tree(arr, (node * 2) + 2, midlle + 1, end));
			return tree[node];
		}
	...
	public int updateNode(int node, int start, int end, int index, int value) {
		...
		if (start == end) {
			...
			//	updateNode(node * 2 + 1, start, midlle, index, value);
				tree[node] = Math.max(updateNode(node * 2 + 1, start, midlle, index, value), tree[2 * node + 2]);
				return tree[node];

			} else if (index > midlle && index <= end) {
				// updateNode(node * 2 + 2, midlle + 1, end, index, value);
				tree[node] = Math.max(tree[2 * node + 1], updateNode(node * 2 + 2, midlle + 1, end, index, value));
				return tree[node];
			}
