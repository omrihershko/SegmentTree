public class MinimumSegmentTreeByArray extends SegmentTreeByArray{
	...
	/**
	 * Constructs of MinimumSegmentTreeByArray
	 ...
	 * @return  int of the min num in the cut array
	 */
 ...
 */
	...
	public int build_tree(int[] arr, int node, int start, int end) {
		if (start == end) {
	        ...
	        tree[node]= Math.min(build_tree(arr, (node * 2) + 1, start, midlle), build_tree(arr, (node * 2) + 2, midlle + 1, end));
	        return tree[node];
	    }
	...
	public int updateNode(int node, int start, int end, int index, int value) {
		...
		if (start == end) {
			...
			if (start <= index && index <= midlle) {
				...
				tree[node] = Math.min(updateNode(node * 2 + 1, start, midlle, index, value), tree[2 * node + 2]);
				return tree[node];

			} else if (index > midlle && index <= end) {
				// updateNode(node * 2 + 2, midlle + 1, end, index, value);
				tree[node] = Math.min(tree[2 * node + 1], updateNode(node * 2 + 2, midlle + 1, end, index, value));
				return tree[node];
			}
